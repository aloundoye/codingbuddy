...
impl CostTracker {
    /// Set pricing based on model name (USD per million tokens).
    /// Falls back to DeepSeek Chat pricing for unknown models.
    pub fn set_pricing_for_model(&mut self, model: &str) {
        let lower = model.to_ascii_lowercase();
        // (input $/M, output $/M, cache_discount_factor)
        let (input, output, cache) = match lower.as_str() {
            ...
            // Anthropic
            m if m.contains("claude-sonnet-4") || m.contains("claude-3-5-sonnet") => {
                (3.00, 15.00, 0.1)
            }
